...
/**
 * float[] 专用排序实现（21 种算法中的适用子集）
 * <p>
 * 与 {@link GenericSorts}（对象数组版）算法一致：严格比较、相同稳定性。
 * 比较语义：Float.compare 全序：NaN 排在最后，-0.0 < 0.0。
 *
 * @author Zero
 */
final class FloatSorts {
     ...
     * 冒泡排序：稳定，带提前退出优化
     */
    static void bubble(float[] a, int from, int to) {
     ...
     * 选择排序：不稳定
     */
    static void selection(float[] a, int from, int to) {
     ...
     */
    static void insertion(float[] a, int from, int to) {
     ...
     */
    static void shell(float[] a, int from, int to) {
        int n = to - from;
        int h = 1;
        while (h < n / 3) {
            h = 3 * h + 1;
     ...
     */
    static void merge(float[] a, int from, int to) {
        float[] aux = new float[to - from];
        mergeRec(a, from, to, aux);
    }

    private static void mergeRec(float[] a, int from, int to, float[] aux) {
        int n = to - from;
     ...
     */
    static void quick(float[] a, int from, int to) {
        quickRec(a, from, to);
    }

    private static void quickRec(float[] a, int from, int to) {
        int n = to - from;
        if (n < INSERTION_THRESHOLD) {
            insertion(a, from, to);
            return;
     ...
     * 堆排序：不稳定，原地最大堆
     */
    static void heap(float[] a, int from, int to) {
        int n = to - from;
     ...
     * 梳排序：不稳定，收缩因子 1.3
     */
    static void comb(float[] a, int from, int to) {
        int n = to - from;
        int gap = n;
     ...
     * 地精排序：稳定
     */
    static void gnome(float[] a, int from, int to) {
        int i = from;
        while (i < to) {
     ...
     * 鸡尾酒排序：稳定，双向冒泡
     */
    static void cocktail(float[] a, int from, int to) {
        int lo = from;
        int hi = to - 1;
        while (lo < hi) {
            boolean swapped = false;
     ...
     */
    static void cycle(float[] a, int from, int to) {
     ...
     * 奇偶排序：稳定，可并行化的冒泡变体
     */
    static void oddEven(float[] a, int from, int to) {
     ...
     * 煎饼排序：不稳定，前缀翻转
     */
    static void pancake(float[] a, int from, int to) {
     ...
     * 臭皮匠排序：不稳定，纯教育用途
     */
    static void stooge(float[] a, int from, int to) {
        if (to - from < 2) {
            return;
        }
     ...
     */
    static void bitonic(float[] a, int from, int to) {
        int n = to - from;
        if (n < 2) {
            return;
        }
     ...
     */
    static void tree(float[] a, int from, int to) {
        int n = to - from;
        if (n < 2) {
            return;
        }
     ...
     */
    static void tim(float[] a, int from, int to) {
        int n = to - from;
        if (n < 2) {
            return;
        }
     ...
     */
    static void bucket(float[] a, int from, int to) {
        int n = to - from;
        if (n < 2) {
            return;
        }
