 ...
 * <p>
 * 通过公共 API（{@link Sort}）对 float[] 的全部 6 种算法做矩阵式验证，
 * 期望结果以 {@link Arrays#sort} 为 oracle（比较语义与实现一致：Float.compare 全序：NaN 排在最后，-0.0 < 0.0）。
 *
 * @author Zero
 */
public class FloatSortsTest {
    ...
    void sortsRandomArray(Algorithm algorithm) {
        float[] a = randomArray(1000, 10000);
    ...
    void sortsSpecialValues(Algorithm algorithm) {
        ...
        float[] a = new float[] {
            ...
            Float.MIN_NORMAL, Float.MAX_VALUE, 1.0f, -1.0f
        };
        float[] expected = a.clone();
        Arrays.sort(expected);
        Sort.sort(a, algorithm);
