 ...
 * int[] 排序测试
 * <p>
 * 通过公共 API（{@link Sort}）对 int[] 的全部适用算法做矩阵式验证，
...
public class IntSortsTest {
    ...
    private static int[] randomSignedArray(int size, int bound) {
        int[] a = new int[size];
        Random random = new Random(SEED);
        for (int i = 0; i < size; i++) {
            a[i] = (int) (random.nextInt(2 * bound) - bound);
        }
        return a;
    }
    ...
    void sortsSignedArray(Algorithm algorithm, int size) {
        int[] a = randomSignedArray(size, 5000);
        int[] expected = a.clone();
        Arrays.sort(expected);
        Sort.sort(a, algorithm);
    ...
    void sortsAlreadySortedArray(Algorithm algorithm, int size) {
        int[] a = randomSignedArray(size, 1000);
        Arrays.sort(a);
        int[] expected = a.clone();
        Sort.sort(a, algorithm);
    ...
    void sortsFullRange(Algorithm algorithm, int size) {
        int[] a = randomSignedArray(100, 1000);
        int[] expected = a.clone();
        Arrays.sort(expected);
    ...
    void defaultSortProducesSortedArray() {
        // 大数组随机输入（走快速/归并分派）
        int[] a = randomSignedArray(1000, 5000);
        int[] expected = a.clone();
        Arrays.sort(expected);
        Sort.sort(a);
